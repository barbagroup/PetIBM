...
int main(int argc, char **argv)
{
	...
	if (nstart == 0) // write initial solution into HDF5 file
	{
		...
		filePath = config["solution"].as<std::string>() +"/"+ ss.str() +".h5";
		ierr = solver.write(t, filePath); CHKERRQ(ierr);

		ierr = PetscPrintf(PETSC_COMM_WORLD, "done\n"); CHKERRQ(ierr);

		iterationsFile = config["directory"].as<std::string>() + "/iterations.txt";
		forcesFile = config["directory"].as<std::string>() + "/forces.txt";
	}
	else // read solution and restart variables from HDF5 file
	{
		ierr = PetscPrintf(
		    PETSC_COMM_WORLD, "[time-step %d] Read solution... ", nstart);
		CHKERRQ(ierr);
		...
		forcesFile = config["directory"].as<std::string>() + 
		             "/forces-" + std::to_string(nstart) + ".txt";
	}
	...
	for (int ite=nstart+1; ite<=nstart+nt; ite++)
	{
		...
		if (ite % nsave == 0)  // write flow solution into HDF5 file
		{
			...
			filePath = config["solution"].as<std::string>() +
			           "/" + ss.str() + ".h5";
			...
			filePath = config["solution"].as<std::string>() +
			           "/" + ss.str() + ".log";
			ierr = petibm::io::writePetscLog(PETSC_COMM_WORLD, filePath); CHKERRQ(ierr);
		}
	}
