...
// required parsing functions are in the header `parser.h` and under the namespace
// `parser`.
# include <petibm/parser.h>

// The header `mesh.h` defines classes and functions regarding mesh objects under
// the namespace `mesh`. And it also defines a data type called `Mesh` to unify
// the member function interface for all different kinds of meshes. This data
// type, `Mesh`, is under the namespace `type`. In other words, with `type::Mesh`,
// high-level CFD code developers don't have to worry about what's the underlying
// mesh. A factory function, `mesh::createMesh`, can create an instance of
// `type::Mesh` with correct underlying mesh types according to the simulation
// configurations provided by end-users.
...
// This header defines classes and functions regarding objects holding solutions.
// A data type called `Solution` is defined under `type` namespace to unify the
// interface to member functions of different kinds of solution structure.
...
int main(int argc, char **argv)
{
    ...
    ierr = petibm::operators::createDivergence(mesh, bd, D, Dbc, PETSC_FALSE); CHKERRQ(ierr);

    // create gradient operator
    ierr = petibm::operators::createGradient(mesh, G, PETSC_FALSE); CHKERRQ(ierr);
    ...
    ierr = MatScale(A, -nu); CHKERRQ(ierr); // use implicit 1st Euler
    ...
    // create BNG (= BN * G)
    ierr = MatMatMult(BN, G, MAT_INITIAL_MATRIX, PETSC_DEFAULT, &BNG); CHKERRQ(ierr);

    // create DBNG (= D * BNG)
    ierr = MatMatMult(D, BNG, MAT_INITIAL_MATRIX, PETSC_DEFAULT, &DBNG); CHKERRQ(ierr);

    // DBNG is a singular matrix in this case, so we utilize PETSc's NullSpace
    {
        MatNullSpace nsp; // a null space object from PETSc
        ierr = MatNullSpaceCreate(PETSC_COMM_WORLD, PETSC_TRUE, 0, nullptr, &nsp); CHKERRQ(ierr);
    ...
    /*
     * Step 6: create vectors for holding values other than velocity and pressure
     */
    ...
    // create bc1; it has the same structure as the velocity, so we just copy the structure
    ierr = VecDuplicate(soln->UGlobal, &bc1); CHKERRQ(ierr);

    // create rhs1; it has the same structure as the velocity, so we just copy the structure
    ierr = VecDuplicate(soln->UGlobal, &rhs1); CHKERRQ(ierr);

    // create rhs2; it has the same structure as the pressure, so we just copy the structure
    ...
    // create vSolver
    ierr = petibm::linsolver::createLinSolver("velocity", config, vSolver); CHKERRQ(ierr);

    // bind the coefficient matrix to vSolver
    ierr = vSolver->setMatrix(A); CHKERRQ(ierr);

    // create pSolver
    ierr = petibm::linsolver::createLinSolver("poisson", config, pSolver); CHKERRQ(ierr);
