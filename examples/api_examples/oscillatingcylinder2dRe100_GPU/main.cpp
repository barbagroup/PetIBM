 ...
 */
PetscErrorCode AppParseBoundaryKinematics(const std::string filepath,
                                          Kinematics &ctx)
{
    YAML::Node config;
 ...
 */
PetscErrorCode AppParseDecouplingParams(const std::string filepath,
                                        DecouplingParams &ctx)
{
    YAML::Node config;
 ...
 */
PetscErrorCode AppSetBoundaryCoordinates(const PetscReal t,
                                         const Kinematics ctx,
                                         petibm::type::SingleBody &body)
{
    PetscFunctionBeginUser;

    PetscReal Xc = -ctx.Am * PetscSinReal(2.0 * PETSC_PI * ctx.f * t), Yc = 0.0;
 ...
 */
PetscErrorCode AppSetBoundaryVelocity(const PetscReal t, const Kinematics ctx,
                                      ...
                                      Vec Ub)
{
    ...
    for (PetscInt k = body->bgPt; k < body->edPt; k++) Ub_arr[k][0] = Ubx;
 ...
 * \param body The immersed body.
 */
PetscErrorCode AppWriteBoundaryCoordinates(const std::string filepath,
                                           const petibm::type::SingleBody body)
{
    PetscErrorCode ierr;
    ...
    for (PetscInt k = 0; k < body->nPts; k++)
    {
        ierr = PetscViewerASCIIPrintf(viewer, "%10.8e\t%10.8e\n",
                                      body->coords[k][0], body->coords[k][1]);
        CHKERRQ(ierr);
    }
...
int main(int argc, char **argv)
{
    ...
    PetscLogStage stageInitialize, stageRHSVelocity, stageSolveVelocity,
        stageRHSForces, stageSolveForces, stageIntegrateForces, stageRHSPoisson,
        stageSolvePoisson, stageUpdate, stageMoveBoundary, stageWrite;
    ...
    ierr = PetscLogStageRegister("initialize", &stageInitialize);
    CHKERRQ(ierr);
    ...
    ierr = AppParseDecouplingParams(config["config.yaml"].as<std::string>(),
                                    decoupling); CHKERRQ(ierr);
    ierr = AppParseBoundaryKinematics(config["config.yaml"].as<std::string>(),
                                      kinematics); CHKERRQ(ierr);
    ...
    ierr = petibm::boundary::createBoundary(mesh, config, boundary);
    CHKERRQ(ierr);
    ...
    ierr = petibm::timeintegration::createTimeIntegration("convection", config,
                                                          convCoeffs);
    CHKERRQ(ierr);
    ierr = petibm::timeintegration::createTimeIntegration("diffusion", config,
                                                          diffCoeffs);
    CHKERRQ(ierr);

    // Assemble the divergence operator and its boundary correction
    ierr = petibm::operators::createDivergence(mesh, boundary, D, DCorrection,
    ...
    ierr = PetscObjectViewFromOptions((PetscObject)D, nullptr, "-D_mat_view");
    CHKERRQ(ierr);
    ...
    ierr = PetscObjectViewFromOptions((PetscObject)G, nullptr, "-G_mat_view");
    CHKERRQ(ierr);

    // Assemble the Laplacian operator and its boundary correction
    ierr = petibm::operators::createLaplacian(mesh, boundary, L, LCorrection);
    CHKERRQ(ierr);
    ...
    ierr = PetscObjectViewFromOptions((PetscObject)L, nullptr, "-L_mat_view");
    CHKERRQ(ierr);
    ...
    ierr = PetscObjectViewFromOptions((PetscObject)A, nullptr, "-A_mat_view");
    CHKERRQ(ierr);
    ...
    ierr = PetscObjectViewFromOptions((PetscObject)BN, nullptr, "-BN_mat_view");
    CHKERRQ(ierr);

    // Assemble the operator to project the velocity of the divergence-free
    // space
    ierr = MatMatMult(BN, G, MAT_INITIAL_MATRIX, PETSC_DEFAULT, &BNG);
    CHKERRQ(ierr);
    ierr = PetscObjectRegisterDestroy((PetscObject)BNG); CHKERRQ(ierr);
    ierr =
        PetscObjectViewFromOptions((PetscObject)BNG, nullptr, "-BNG_mat_view");
    CHKERRQ(ierr);
    ...
    // Set the nullspace for Poisson system with Neumann conditions
    {
        ...
        {
            MatNullSpace nsp;
            ierr = MatNullSpaceCreate(mesh->comm, PETSC_TRUE, 0, nullptr, &nsp);
            CHKERRQ(ierr);
    ...
    ierr = petibm::operators::createConvection(mesh, boundary, N);
    CHKERRQ(ierr);
    ierr = PetscObjectRegisterDestroy((PetscObject)N); CHKERRQ(ierr);
    ierr = PetscObjectViewFromOptions((PetscObject)N, nullptr, "-N_mat_view");
    CHKERRQ(ierr);
    ...
    ierr = PetscViewerFileSetName(viewerIters,
                                  (directory + "/iterations.txt").c_str());
    CHKERRQ(ierr);
    ...
    ierr = PetscViewerFileSetMode(viewerForces, FILE_MODE_WRITE);
    CHKERRQ(ierr);
    ierr = PetscViewerFileSetName(viewerForces,
                                  (directory + "/forces.txt").c_str());
    CHKERRQ(ierr);
    ierr = PetscObjectRegisterDestroy((PetscObject)viewerForces);
    CHKERRQ(ierr);
    ...
    ierr = petibm::linsolver::createLinSolver("forces", config, fSolver);
    CHKERRQ(ierr);
    ...
    ierr = PetscPrintf(PETSC_COMM_WORLD, "Initialization complete!\n");
    CHKERRQ(ierr);
    ...
    ierr = PetscPrintf(PETSC_COMM_WORLD, "[time-step 0] Writing solution ... ");
    CHKERRQ(ierr);
    ierr = solution->write(directory + "/solution/0000000"); CHKERRQ(ierr);
    ierr = AppWriteBoundaryCoordinates(directory + "/solution/0000000.curve",
                                       body); CHKERRQ(ierr);
    ierr = PetscPrintf(PETSC_COMM_WORLD, "done\n"); CHKERRQ(ierr);
    ierr = PetscLogStagePop(); CHKERRQ(ierr);

    ierr = PetscLogStageRegister("set velocity RHS", &stageRHSVelocity);
    CHKERRQ(ierr);
    ierr = PetscLogStageRegister("solve velocity", &stageSolveVelocity);
    CHKERRQ(ierr);
    ierr = PetscLogStageRegister("set forces RHS", &stageRHSForces);
    CHKERRQ(ierr);
    ierr = PetscLogStageRegister("solve forces", &stageSolveForces);
    CHKERRQ(ierr);
    ierr = PetscLogStageRegister("set Poisson RHS", &stageRHSPoisson);
    CHKERRQ(ierr);
    ierr = PetscLogStageRegister("solve Poisson", &stageSolvePoisson);
    CHKERRQ(ierr);
    ierr = PetscLogStageRegister("update", &stageUpdate); CHKERRQ(ierr);
    ierr = PetscLogStageRegister("integrate forces", &stageIntegrateForces);
    CHKERRQ(ierr);
    ierr = PetscLogStageRegister("move boundary", &stageMoveBoundary);
    CHKERRQ(ierr);
    ...
    for (PetscInt ite = nstart + 1; ite <= nstart + nt; ite++)
    {
        ...
        ierr = petibm::operators::createDelta(mesh, boundary, bodyPack, E);
        CHKERRQ(ierr);
        ...
        // Solve the forcing system to obtain prediction of the Lagrangian
        // forces
        ...
        ierr = MatMultAdd(LCorrection, solution->UGlobal, diff[0], diff[0]);
        CHKERRQ(ierr);
        ...
        ierr = MatMultAdd(BNG, dp, solution->UGlobal, solution->UGlobal);
        CHKERRQ(ierr);
        ierr = VecScale(solution->UGlobal, -1.0); CHKERRQ(ierr);
        ...
        ierr = PetscLogStagePush(stageWrite); CHKERRQ(ierr);
        ierr = PetscViewerASCIIPrintf(viewerIters, "%d\t%d\t", ite, inner);
        CHKERRQ(ierr);
        ierr = PetscViewerASCIIPrintf(viewerIters, "%d\t%e\t", nIters_v / inner,
                                      res_v / inner); CHKERRQ(ierr);
        ierr = PetscViewerASCIIPrintf(viewerIters, "%d\t%e\t", nIters_p, res_p);
        CHKERRQ(ierr);
        ierr = PetscViewerASCIIPrintf(viewerIters, "%d\t%e\n", nIters_f, res_f);
        CHKERRQ(ierr);
        ierr = PetscViewerFileSetMode(viewerIters, FILE_MODE_APPEND);
        CHKERRQ(ierr);
        ierr = PetscLogStagePop(); CHKERRQ(ierr);

        if (ite % nsave == 0)
        {
            // Write the Eulerian field solution and the body coordinates into
            // files
            ierr = PetscLogStagePush(stageWrite); CHKERRQ(ierr);
            ierr = PetscPrintf(PETSC_COMM_WORLD,
            ...
            ierr = AppWriteBoundaryCoordinates(directory + ss.str() + ".curve",
                                               body); CHKERRQ(ierr);
            ierr = PetscLogStagePop(); CHKERRQ(ierr);
            ierr = PetscViewerFileSetName(
                viewerLog, (directory + ss.str() + ".log").c_str());
            CHKERRQ(ierr);
        ...
        ierr = PetscViewerASCIIPrintf(viewerForces, "%10.8e", t);
        CHKERRQ(ierr);
        for (int d = 0; d < mesh->dim; d++)
        {
            ierr = PetscViewerASCIIPrintf(viewerForces, "\t%10.8e", F[0][d]);
            CHKERRQ(ierr);
        }
        ierr = PetscViewerASCIIPrintf(viewerForces, "\n"); CHKERRQ(ierr);
        ierr = PetscViewerFileSetMode(viewerForces, FILE_MODE_APPEND);
        CHKERRQ(ierr);
        ierr = PetscLogStagePop(); CHKERRQ(ierr);
    }
