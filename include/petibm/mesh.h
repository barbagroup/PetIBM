 ...
 * derived from the base class: \ref petibm::mesh::CartesianMesh
 * "CartesianMesh".
 *
 * The design of PetIBM is to use `std::shared_ptr` to hold the base class for
 * instances of different kinds of derived classes, so that we can call the
 * public APIs regardless the types of the derived classes. \ref
 * petibm::type::Mesh "Mesh" is defined as a shared pointer to \ref
 * petibm::mesh::MeshBase "MeshBase". Users should use \ref petibm::type::Mesh
 * "Mesh" instead of using classes directly. And users should also initialize
 * mesh instances with the factory function, \ref petibm::mesh::createMesh
 * "createMesh", instead of initializing the instances directly.
 *
...
public:
    ...
    /** \brief The beginning index of all fields in all directions of this
     * process. */
    type::IntVec2D bg;
    ...
    /** \brief The number of points of all fields in all directions of this
     * process. */
    type::IntVec2D m;
    ...
    /**
     * \brief Write the mesh object into a HDF5 file.
     *
     ...
     * \param s [in] MatStencil object of target point.
     ...
     * \param f [in] Target field (u=0, v=1, w=2, p=3).
     * \param s [in] MatStencil object of target point.
     * \param idx [out] Global index in unpacked DM object.
     *
     * If the provided MatStencil is not valid or is a ghost point, the `idx`
     * will be `-1`.
     */
...
protected:
     ...
     * \see petibm::mesh::createMesh
     */
    virtual PetscErrorCode init(const MPI_Comm &world,
 ...
 * \param node [in] YAML configuration node.
 * \param mesh [out] Structured Cartesian mesh object.
 *
