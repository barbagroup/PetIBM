...
AMGX_LIB_RPATH=-Wl,-rpath=`find $AMGX_DIR -name libamgxsh.so | sed -e 's/\/libamgxsh.so//g'`
AMGX_LIBRARY=-lamgxsh

CPPFLAGS_PREPEND($AMGX_INC_PATH)
LDFLAGS_PREPEND($AMGX_LIB_PATH $AMGX_LIB_RPATH)
...
PACKAGE_LDFLAGS_PREPEND($AMGX_LIB_PATH $AMGX_LIB_RPATH)
