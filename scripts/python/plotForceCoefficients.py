"""
Plots the instantaneous forces or force coefficients.
"""
...
class ForceCoefficient(object):
  ...
  def get_fluctuations(self):
    ...
    """
    self.fluctuations = numpy.absolute([self.values[self.minima[-1]]
                                        - self.mean,
                                        self.values[self.maxima[-1]]
                                        - self.mean])
  ...
  def get_extrema(self, order=5):
    ...
    order: integer, optional
      ...
      default: 5.
    """
    minima = signal.argrelextrema(self.values,
                                  numpy.less_equal,
                                  order=order)[0][:-1]
    maxima = signal.argrelextrema(self.values,
                                  numpy.greater_equal,
                                  order=order)[0][:-1]
...
class Body(object):
  ...
  def __init__(self, name, forces, coefficient=1.0):
    ...
    name: string
      Name of the body.
    forces: list of 1D arrays of floats
      Force in the x-, y-  and z-(if applicable) directions.
    coefficient: float, optional
      Number to convert a force into a force coefficient;
      default: 1.0.
    """
    self.name = name
  ...
  def get_extrema(self, order=5):
    ...
    order: float, optional
      Parameters used for comparison to define an extremum;
      default: 5.
    """
    for i in xrange(len(self.force_coefficients)):
  ...
  def print_info(self):
    ...
    for i, coefficient in enumerate(self.force_coefficients):
      try:
        ...
        print('\t<{}> = {:0.3f}'.format(names[i], coefficient.mean))
...
class Case(object):
  ...
  def __init__(self, parameters):
    ...
    parameters: namespace
      Database with arguments parsed from the command-line.
    """
    print('\nCase: {}'.format(parameters.directory))
  ...
  def read_forces(self):
    ...
    """
    print('\nReading forces ...')
    forces_path = os.path.join(self.parameters.directory, 'forces.txt')
    ...
    time_limits = self.parameters.time_limits
    mask = numpy.where(numpy.logical_and(t >= time_limits[0],
                                         t <= time_limits[1]))[0]
    ...
    if not self.parameters.body_names:
      self.parameters.body_names = ['body {}'.format(i + 1)
                                    for i in xrange(n_bodies)]
    # create Body objects that store the forces of each body
    print('\nInfo: ForceCoefficient = {} x Force'
          ''.format(self.parameters.coefficient))
  ...
  def get_properties(self):
    ...
    the Strouhal number and the extrema indices, for each body.
    """
    ...
    print('\nConsidering force coefficients between {} and {}:'
          ''.format(self.times[mask[0]], self.times[mask[-1]]))
  ...
  def plot_force_coefficients(self):
    ...
    pyplot.axis(self.parameters.plot_limits)
    # save the image as .png file
    if self.parameters.save:
      images_directory = os.path.join(self.parameters.directory, 'images')
      if not os.path.isdir(images_directory):
        os.makedirs(images_directory)
      pyplot.savefig(os.path.join(images_directory,
                                  self.parameters.image_name + '.png'))
      print('\nFigure saved in folder {}'
            ''.format(os.path.basename(images_directory)))
    # dislay the figure
