...
def parse_command_line():
  ...
  parser.add_argument('--time-step', '-ts',
                      dest='time_step',
                      type=int,
                      default=1000,
  ...
  parser.add_argument('--amplitude', '-a',
                      ...
                      type=float,
                      default=1.0,
                      help='amplitude of the Taylor-Green vortex')
...
def l2_norm(field):
  ...
  return numpy.linalg.norm(field[j_start:j_end:j_stride,
                                 i_start:i_end:i_stride])


def compute_order(ratio, coarse, medium, fine):
  """
  Computes the observed order of convergence using the solution on three grids.

  Parameters
  ----------
  ratio: float
    Grid-refinement ratio.
  coarse, medium, fine: 3 2D arrays of floats
...
def restriction(fine, coarse):
  ...
  Parameters
  ----------
  fine, coarse: 2 Field objects
    Fine and coarse numerical solutions.

  Returns
  -------
  fine_on_coarse: Field object
  ...
  values = numpy.array([fine.values[j][mask_x] for j in xrange(fine.y.size)
                        if mask_y[j]])
...
def taylor_green_vortex(x, y,
  ...
  Parameters
  ----------
  x, y: 2 1D arrays of floats
    Coordinates in the x- and y- directions.
  x_start, x_end, y_start, y_end: 4 floats, optional
    Limits of the physical domain;
  ...
  # u-velocity
  u = (-V * numpy.cos(X) * numpy.sin(Y)
       * math.exp(-2.0 * (2.0 * math.pi)**2 * time / Re))
  # v-velocity
  v = (+V * numpy.sin(X) * numpy.cos(Y)
       * math.exp(-2.0 * (2.0 * math.pi)**2 * time / Re))
  # pressure
  p = (-0.25 * (numpy.cos(2.0 * X) + numpy.cos(2.0 * Y))
       * math.exp(-4.0 * (2.0 * math.pi)**2 * time / Re))
  # z-vorticity
  w = (2.0 * numpy.sin(X) * numpy.sin(Y)
       * math.exp(-2.0 * (2.0 * math.pi)**2 * time / Re))
  return u, v, p, w
...
def plot_field(x, y, u, name, image_path):
  ...
  fig.colorbar(cont, label='{}'.format(name), fraction=0.046, pad=0.04)
...
def main(args):
  ...
  for i, case in enumerate(cases):
    ...
    cases[i]['u'], cases[i]['v'] = ioPetIBM.read_velocity(args.time_step,
                                                          [x, y],
                                                          periodic=['x', 'y'],
                                                          directory=directory)
    ...
    if args.plot:
      ...
      images_directory = os.path.join(case['directory'],
                                      'images',
                                      'differences')
  ...
  if args.save or args.show:
    ...
    h = numpy.linspace(cases[0]['grid-spacing'],
                       cases[-1]['grid-spacing'],
                       101)
    pyplot.plot(h, h, label='$1^{st}$-order convergence',
                color='k')
