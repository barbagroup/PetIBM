                                    ...
                                    type::RealVec2D &coords)
{
    ...
    if (std::getline(inFile, line))
    {
        ...
        if (sline.peek() != EOF)
            SETERRQ1(
                PETSC_COMM_WORLD, PETSC_ERR_FILE_READ,
...
PetscErrorCode print(const std::string &info)
{
    ...
    ierr = PetscSynchronizedPrintf(PETSC_COMM_WORLD, info.c_str());
    CHKERRQ(ierr);
                             ...
                             const PetscFileMode mode)
{
    ...
    {
        ierr = PetscObjectSetName((PetscObject)vecs[i], names[i].c_str());
        CHKERRQ(ierr);
                             ...
                             const PetscFileMode mode)
{
    ...
    for (unsigned int i = 0; i < vecs.size(); i++)
    {
        Vec temp;
        ierr =
            VecCreateMPIWithArray(comm, 1, n[i], PETSC_DECIDE, nullptr, &temp);
        CHKERRQ(ierr);
        ierr = PetscObjectSetName((PetscObject)temp, names[i].c_str());
        CHKERRQ(ierr);
                             ...
                             const PetscFileMode mode)
{
    ...
    {
        Vec temp;
        ierr = VecCreateMPIWithArray(comm, 1, vecs[i].size(), PETSC_DECIDE,
                                     nullptr, &temp); CHKERRQ(ierr);
        ierr = PetscObjectSetName((PetscObject)temp, names[i].c_str());
        CHKERRQ(ierr);
...
PetscErrorCode writePetscLog(const MPI_Comm comm, const std::string &filePath)
{
    ...
    ierr = PetscViewerCreate(comm, &viewerLog); CHKERRQ(ierr);
    ...
    ierr = PetscViewerFileSetName(viewerLog, filePath.c_str()); CHKERRQ(ierr);
