                            ...
                            Mat &BnHead)
{
    ...
    if (n < 1)
        SETERRQ(PETSC_COMM_WORLD, 56,
                "The order of Bn can not be smaller than 1.");
    ...
    ierr = MatSetOption(BnHead, MAT_KEEP_NONZERO_PATTERN, PETSC_FALSE);
    CHKERRQ(ierr);
    ierr = MatSetOption(BnHead, MAT_IGNORE_ZERO_ENTRIES, PETSC_TRUE);
    CHKERRQ(ierr);
    ...
    ierr = MatMPIAIJSetPreallocation(BnHead, 1, nullptr, 0, nullptr);
    CHKERRQ(ierr);
    ...
    for (PetscInt term = 2; term <= n; ++term)
    {
        ...
        for (PetscInt c = 2; c < term; ++c)
        {
            ...
            ierr = MatMatMult(Op, rightMat, MAT_INITIAL_MATRIX, PETSC_DEFAULT,
                              &tempResult); CHKERRQ(ierr);
                        ...
                        const PetscReal &coeff, const PetscInt &n, Mat &Bn)
{
    ...
    ierr = MatSetOption(Bn, MAT_KEEP_NONZERO_PATTERN, PETSC_FALSE);
    CHKERRQ(ierr);
    ierr = MatSetOption(Bn, MAT_IGNORE_ZERO_ENTRIES, PETSC_TRUE);
    CHKERRQ(ierr);
    ...
    ierr = MatMPIAIJSetPreallocation(Bn, 1, nullptr, 0, nullptr);
    CHKERRQ(ierr);
    ...
    {
        // the most right matrix of successive mat-mat-multiplication
        ierr = MatDuplicate(leftMat, MAT_COPY_VALUES, &rightMat);
        CHKERRQ(ierr);
