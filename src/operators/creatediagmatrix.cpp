 ...
 * \brief Definitions of functions creating different kinds of diagonal
 * matrices. \copyright Copyright (c) 2016-2018, Barba group. All rights
                                ...
                                const std::vector<KernelType> &kernels, Mat &M)
{
    ...
    ierr = MatSetSizes(M, mesh->UNLocal, mesh->UNLocal, PETSC_DETERMINE,
    ...
    ierr = MatSetOption(M, MAT_KEEP_NONZERO_PATTERN, PETSC_FALSE);
    CHKERRQ(ierr);
    ...
    for (PetscInt field = 0; field < mesh->dim; ++field)
        for (PetscInt k = mesh->bg[field][2]; k < mesh->ed[field][2]; ++k)
            for (PetscInt j = mesh->bg[field][1]; j < mesh->ed[field][1]; ++j)
                for (PetscInt i = mesh->bg[field][0]; i < mesh->ed[field][0];
                     ++i)
                {
                    PetscInt idx;
                    ...
                    ierr = mesh->getPackedGlobalIndex(field, self, idx);
                    CHKERRQ(ierr);
...
PetscErrorCode createRInv(const type::Mesh &mesh, Mat &RInv)
{
    ...
    kernel[0] = [&mesh](const PetscInt &i, const PetscInt &j,
                        const PetscInt &k) -> PetscReal {
        return 1.0 / (mesh->dL[0][1][j] * mesh->dL[0][2][k]);
    };
    kernel[1] = [&mesh](const PetscInt &i, const PetscInt &j,
                        const PetscInt &k) -> PetscReal {
        return 1.0 / (mesh->dL[1][0][i] * mesh->dL[1][2][k]);
    };
    kernel[2] = [&mesh](const PetscInt &i, const PetscInt &j,
                        const PetscInt &k) -> PetscReal {
        return 1.0 / (mesh->dL[2][0][i] * mesh->dL[2][1][j]);
    };
...
PetscErrorCode createMHead(const type::Mesh &mesh, Mat &MHead)
{
    ...
    kernel[0] = [&mesh](const PetscInt &i, const PetscInt &j,
                        const PetscInt &k) -> PetscReal {
        return mesh->dL[0][0][i];
    };
    kernel[1] = [&mesh](const PetscInt &i, const PetscInt &j,
                        const PetscInt &k) -> PetscReal {
        return mesh->dL[1][1][j];
    };
    kernel[2] = [&mesh](const PetscInt &i, const PetscInt &j,
                        const PetscInt &k) -> PetscReal {
        return mesh->dL[2][2][k];
    };
...
PetscErrorCode createM(const type::Mesh &mesh, Mat &M)
{
    ...
    kernel[0] = [&mesh](const PetscInt &i, const PetscInt &j,
                        const PetscInt &k) -> PetscReal {
        return mesh->dL[0][0][i] / (mesh->dL[0][1][j] * mesh->dL[0][2][k]);
    };
    kernel[1] = [&mesh](const PetscInt &i, const PetscInt &j,
                        const PetscInt &k) -> PetscReal {
        return mesh->dL[1][1][j] / (mesh->dL[1][0][i] * mesh->dL[1][2][k]);
    };
    kernel[2] = [&mesh](const PetscInt &i, const PetscInt &j,
                        const PetscInt &k) -> PetscReal {
        return mesh->dL[2][2][k] / (mesh->dL[2][0][i] * mesh->dL[2][1][j]);
    };
...
PetscErrorCode createIdentity(const type::Mesh &mesh, Mat &I)
{
    ...
    // set kernels
    kernel[0] = [&mesh](const PetscInt &i, const PetscInt &j,
                        const PetscInt &k) -> PetscReal { return 1.0; };
    kernel[1] = [&mesh](const PetscInt &i, const PetscInt &j,
                        const PetscInt &k) -> PetscReal { return 1.0; };
    kernel[2] = [&mesh](const PetscInt &i, const PetscInt &j,
                        const PetscInt &k) -> PetscReal { return 1.0; };
