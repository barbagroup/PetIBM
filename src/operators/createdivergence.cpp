...
typedef std::function<MatStencil(const PetscInt &, const PetscInt &,
                                 const PetscInt &)>
    GetNeighborFunc;

// a function type for functions returning the value of a matrix entry
typedef std::function<PetscReal(const PetscInt &, const PetscInt &,
                                const PetscInt &)>
    Kernel;
...
PetscErrorCode createDivergence(const type::Mesh &mesh,
                                ...
                                Mat &DCorrection, const PetscBool &normalize)
{
    PetscFunctionBeginUser;
    ...
    // set up getNeighbor
    getNeighbor[0] = [](const PetscInt &i, const PetscInt &j,
                        const PetscInt &k) -> MatStencil {
        return {k, j, i - 1};
    };
    getNeighbor[1] = [](const PetscInt &i, const PetscInt &j,
                        const PetscInt &k) -> MatStencil {
        return {k, j - 1, i};
    };
    getNeighbor[2] = [](const PetscInt &i, const PetscInt &j,
                        const PetscInt &k) -> MatStencil {
        return {k - 1, j, i};
    };
    ...
    {
        kernel[0] = [&mesh](const PetscInt &i, const PetscInt &j,
                            const PetscInt &k) -> PetscReal {
            return mesh->dL[0][1][j] * mesh->dL[0][2][k];
        };
        kernel[1] = [&mesh](const PetscInt &i, const PetscInt &j,
                            const PetscInt &k) -> PetscReal {
            return mesh->dL[1][0][i] * mesh->dL[1][2][k];
        };
        kernel[2] = [&mesh](const PetscInt &i, const PetscInt &j,
                            const PetscInt &k) -> PetscReal {
            return mesh->dL[2][0][i] * mesh->dL[2][1][j];
        };
    }

    // create matrix
    ierr = MatCreate(mesh->comm, &D); CHKERRQ(ierr);
    ierr = MatSetSizes(D, mesh->pNLocal, mesh->UNLocal, PETSC_DETERMINE,
    ...
    ierr = MatMPIAIJSetPreallocation(D, mesh->dim * 2, nullptr, 3, nullptr);
    CHKERRQ(ierr);
    ...
    for (PetscInt k = mesh->bg[3][2]; k < mesh->ed[3][2]; ++k)
        for (PetscInt j = mesh->bg[3][1]; j < mesh->ed[3][1]; ++j)
            for (PetscInt i = mesh->bg[3][0]; i < mesh->ed[3][0]; ++i)
            {
                ...
                ierr = mesh->getGlobalIndex(3, {k, j, i, 0}, self);
                CHKERRQ(ierr);

                // calculate and set values of the two surfaces in each
                // direction
                for (PetscInt field = 0; field < mesh->dim; ++field)
                {
                    ...
                    ierr = mesh->getPackedGlobalIndex(field, target, targetIdx);
                    CHKERRQ(ierr);

                    ierr =
                        MatSetValue(D, self, targetIdx, value, INSERT_VALUES);
                    CHKERRQ(ierr);
        ...
        for (auto &bd : bc->bds[f])
            if (bd->onThisProc)
                for (auto &pt : bd->points)
                {
                    ...
                    ierr = MatSetValue(D, ctx->modifier[f][pt.first].row, col,
