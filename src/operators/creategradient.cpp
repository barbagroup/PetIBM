...
typedef std::function<type::RealVec1D(const PetscInt &, const PetscInt &,
                                      const PetscInt &)>
    Kernel;
                              ...
                              const PetscBool &normalize)
{
    ...
    ierr = MatSetSizes(G, mesh->UNLocal, mesh->pNLocal, PETSC_DETERMINE,
    ...
    ierr = MatSetOption(G, MAT_KEEP_NONZERO_PATTERN, PETSC_FALSE);
    CHKERRQ(ierr);
        ...
        for (PetscInt k = mesh->bg[field][2]; k < mesh->ed[field][2]; ++k)
            for (PetscInt j = mesh->bg[field][1]; j < mesh->ed[field][1]; ++j)
                     ...
                     ++i)
                {
                    ...
                    ierr = mesh->getPackedGlobalIndex(field, loc[0], rId);
                    CHKERRQ(ierr);

                    // loop through columns
                    for (PetscInt n = 0; n < 2; ++n)
                    {
                        ierr = mesh->getPackedGlobalIndex(3, loc[n], cId);
                        CHKERRQ(ierr);

                        ierr =
                            MatSetValue(G, rId, cId, values[n], INSERT_VALUES);
                        CHKERRQ(ierr);
                    }
                }
