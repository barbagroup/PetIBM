...
void NavierStokesSolver<2>::generateDiagonalMatrices()
{
	...
	ierr = DMCompositeGetAccess(pack, MHat, &MHatxGlobal, &MHatyGlobal); CHKERRV(ierr);
	ierr = DMCompositeGetAccess(pack, RInv, &RInvxGlobal, &RInvyGlobal); CHKERRV(ierr);
	ierr = DMCompositeGetAccess(pack, BN,   &BNxGlobal,   &BNyGlobal); CHKERRV(ierr);
	...
	ierr = DMCompositeRestoreAccess(pack, MHat, &MHatxGlobal, &MHatyGlobal); CHKERRV(ierr);
	ierr = DMCompositeRestoreAccess(pack, RInv, &RInvxGlobal, &RInvyGlobal); CHKERRV(ierr);
	ierr = DMCompositeRestoreAccess(pack, BN,   &BNxGlobal,   &BNyGlobal); CHKERRV(ierr);
}
...
void NavierStokesSolver<3>::generateDiagonalMatrices()
{
	...
	ierr = DMCompositeGetAccess(pack, MHat, &MHatxGlobal, &MHatyGlobal, &MHatzGlobal); CHKERRV(ierr);
	ierr = DMCompositeGetAccess(pack, RInv, &RInvxGlobal, &RInvyGlobal, &RInvzGlobal); CHKERRV(ierr);
	ierr = DMCompositeGetAccess(pack, BN,   &BNxGlobal,   &BNyGlobal,   &BNzGlobal); CHKERRV(ierr);
	...
	ierr = DMCompositeRestoreAccess(pack, MHat, &MHatxGlobal, &MHatyGlobal, &MHatzGlobal); CHKERRV(ierr);
	ierr = DMCompositeRestoreAccess(pack, RInv, &RInvxGlobal, &RInvyGlobal, &RInvzGlobal); CHKERRV(ierr);
	ierr = DMCompositeRestoreAccess(pack, BN,   &BNxGlobal,   &BNyGlobal,   &BNzGlobal); CHKERRV(ierr);
}