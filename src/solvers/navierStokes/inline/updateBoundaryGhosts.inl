...
PetscErrorCode NavierStokesSolver<2>::updateBoundaryGhosts()
{
  ...
  bool periodicX = (flow->boundaries[XMINUS][0].type == PERIODIC), 
       periodicY = (flow->boundaries[YMINUS][0].type == PERIODIC);
  ...
  if (!periodicX) // do not update if x-periodic
  {
      ...
      if (mstart == 0) // if left boundary on current process
      {
        switch (flow->boundaries[XMINUS][0].type)
        {
          case DIRICHLET:
            ...
            break;
          case CONVECTIVE: 
            beta = flow->boundaries[XMINUS][0].value*dt/dxU[0];
      ...
      {     
        ...
        {         
          case DIRICHLET:
            ...
            break;
          case CONVECTIVE:
            beta = flow->boundaries[XPLUS][0].value*dt/dxU[M];
  ...
  if (!periodicY) // do not update if y-periodic
  {
      ...
      if (nstart == 0) // bottom boundary on current process
      {
        switch (flow->boundaries[YMINUS][0].type)
        {
          case DIRICHLET:
            ...
            break;
          case CONVECTIVE:
            beta = flow->boundaries[YMINUS][1].value*dt/dyU[0];
      ...
      if (nstart+n-1 == N-1) // top boundary on current process
      {
        switch (flow->boundaries[YPLUS][0].type)
        {
          case DIRICHLET:
            ...
            break;
          case CONVECTIVE:
            beta = flow->boundaries[YPLUS][1].value*dt/dyU[N];
      ...
      if (mstart == 0) // left boundary on current process
      {
        switch (flow->boundaries[XMINUS][1].type)
        {
          case DIRICHLET:
            ...
            break;
          case CONVECTIVE: 
            beta = flow->boundaries[XMINUS][0].value*dt/dxV[0];
      ...
      if (mstart+m-1 == M-1) // right boundary on current process
      {
        switch (flow->boundaries[XPLUS][1].type)
        {
          case DIRICHLET:
            ...
            break;
          case CONVECTIVE:
            beta = flow->boundaries[XPLUS][0].value*dt/dxV[M];
      ...
      if (nstart == 0) // bottom boundary on current process
      {
        switch (flow->boundaries[YMINUS][1].type)
        {
          case DIRICHLET:
            ...
            break;
          case CONVECTIVE:
            beta = flow->boundaries[YMINUS][1].value*dt/dyV[0];
      ...
      if (nstart+n-1 == N-1) // top boundary on current process
      {
        switch (flow->boundaries[YPLUS][1].type)
        {
          case DIRICHLET:
            ...
            break;
          case CONVECTIVE:
            beta = flow->boundaries[YPLUS][1].value*dt/dyV[N];
...
PetscErrorCode NavierStokesSolver<3>::updateBoundaryGhosts()
{
  ...
  // look for periodic boundary conditions in all directions
  bool periodicX = (flow->boundaries[XMINUS][0].type == PERIODIC), 
       periodicY = (flow->boundaries[YMINUS][0].type == PERIODIC),
       periodicZ = (flow->boundaries[ZMINUS][0].type == PERIODIC);
  ...
  if (!periodicZ) // do not update if z-periodic
  {
    for (j=nstart; j<nstart+n; j++) // loop over y-direction
    {
      for (i=mstart; i<mstart+m; i++) // loop over x-direction
        ...
        if (pstart == 0) // back boundary on current process
        {
          switch (flow->boundaries[ZMINUS][0].type)
          {
            case DIRICHLET:
              ...
              break;
            case CONVECTIVE:
              beta = flow->boundaries[ZMINUS][2].value*dt/dzU[0];
        ...
        if (pstart+p-1 == P-1) // fron boundary on current process
        {
          switch (flow->boundaries[ZPLUS][0].type)
          {
            case DIRICHLET:
              ...
              break;
            case CONVECTIVE:
              beta = flow->boundaries[ZPLUS][2].value*dt/dzU[P];
        ...
        if (pstart == 0) // back boundary on current process
        {
          switch (flow->boundaries[ZMINUS][1].type)
          {
            case DIRICHLET:
              ...
              break;
            case CONVECTIVE:
              beta = flow->boundaries[ZMINUS][2].value*dt/dzV[0];
        ...
        if (pstart+p-1 == P-1) // front boundary on current process
        {
          switch (flow->boundaries[ZPLUS][1].type)
          {
            case DIRICHLET:
              ...
              break;
            case CONVECTIVE:
              beta = flow->boundaries[ZPLUS][2].value*dt/dzV[P];
        ...
        if (mstart == 0) // left boundary on current process
        {
          switch (flow->boundaries[XMINUS][2].type)
          {
            case DIRICHLET:
              ...
              break;
            case CONVECTIVE:
              beta = flow->boundaries[XMINUS][0].value*dt/dxW[0];
        ...
        if (mstart+m-1 == M-1) // right boundary on current process
        {
          switch (flow->boundaries[XPLUS][2].type)
          {
            case DIRICHLET:
              ...
              break;
            case CONVECTIVE:
              beta = flow->boundaries[XPLUS][0].value*dt/dxW[M];
        ...
        if (nstart == 0) // bottom boundary on current process
        {
          switch (flow->boundaries[YMINUS][2].type)
          {
            case DIRICHLET:
              ...
              break;
            case CONVECTIVE:
              beta = flow->boundaries[YMINUS][1].value*dt/dyW[0];
        ...
        if (nstart+n-1 == N-1) // top boundary on current process
        {
          switch (flow->boundaries[YPLUS][2].type)
          {
            case DIRICHLET:
              ...
              break;
            case CONVECTIVE:
              beta = flow->boundaries[YPLUS][1].value*dt/dyW[N];
        ...
        if (pstart == 0) // back boundary on current process
        {
          switch (flow->boundaries[ZMINUS][2].type)
          {
            case DIRICHLET:
              ...
              break;
            case CONVECTIVE:
              beta = flow->boundaries[ZMINUS][2].value*dt/dzW[0];
        ...
        if (pstart+p-1 == P-1) // front boundary on current process
        {
          switch (flow->boundaries[ZPLUS][2].type)
          {
            case DIRICHLET:
              ...
              break;
            case CONVECTIVE:
              beta = flow->boundaries[ZPLUS][2].value*dt/dzW[P];
