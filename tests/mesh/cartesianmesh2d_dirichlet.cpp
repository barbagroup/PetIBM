...
class CartesianMeshTest2D_AllDirichlet : public ::testing::Test
{
protected:
    CartesianMeshTest2D_AllDirichlet(){};

    virtual ~CartesianMeshTest2D_AllDirichlet(){};
    ...
    static void SetUpTestCase()
    {
        ...
        ierr = DMDACreate2d(PETSC_COMM_WORLD, DM_BOUNDARY_GHOSTED,
                            DM_BOUNDARY_GHOSTED, DMDA_STENCIL_BOX, 12, 10,
...
petibm::type::Mesh CartesianMeshTest2D_AllDirichlet::mesh = nullptr;
DM CartesianMeshTest2D_AllDirichlet::da[5]{nullptr, nullptr, nullptr, nullptr,
                                           nullptr};

// test dimension
TEST_F(CartesianMeshTest2D_AllDirichlet, check_dim) { ASSERT_EQ(2, mesh->dim); }
...
TEST_F(CartesianMeshTest2D_AllDirichlet, check_n)
{
    petibm::type::RealVec2D n{
        {11, 11, 1}, {12, 10, 1}, {1, 1, 1}, {12, 11, 1}, {13, 12, 1}};
...
{
    petibm::type::BoolVec2D periodic{{PETSC_FALSE, PETSC_FALSE, PETSC_FALSE},
    ...
    for (int f = 0; f < 5; ++f)
    {
        ...
        for (int k = 0; k < mesh->n[f][2]; k++)
            for (int j = 0; j < mesh->n[f][1]; j++)
                for (int i = 0; i < mesh->n[f][0]; i++)
                {
                    ...
                    ASSERT_EQ(idx2, idx)
                        << "getNatrualIndex  wrong at field " << f << " and ("
                        << i << ", " << j << ", " << k << ") " << std::endl;
                }
        ...
        for (int k = 0; k < mesh->n[f][2]; k++)
            for (int i = 0; i < mesh->n[f][0]; i++)
            {
                ...
                mesh->getNaturalIndex(f, i, j, k, idx);
                ASSERT_EQ(-1, idx)
                    << "getNatrualIndex  wrong at field " << f << " and (" << i
                    << ", " << j << ", " << k << ") " << std::endl;
        ...
        for (int k = 0; k < mesh->n[f][2]; k++)
        {
            ...
            ASSERT_EQ(-1, idx)
                << "getNatrualIndex  wrong at field " << f << " and (" << -1
                << ", " << -1 << ", " << k << ") " << std::endl;
